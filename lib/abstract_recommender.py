...
class AbstractRecommender(object):
    ...
    def _get_options_suffix(self):
        ...
        for option, value in self.options.items():
            suffix+="_{}_{}".format(option,value)
        return suffix+"_{}_{}_{}_{}_{}_{}".format("Rec",self.config.get_recommender(), "Factors",self.n_factors, "Lambda",self._lambda)
    ...
    def get_evaluation_report(self):
        ...
        """
        recalls, ndcgs, mrrs = self.evaluator.recall_ndcg_mrr_at_list_of_x(list(range(20, 201, 20)), [5, 10], [5, 10],
                                                                           self.test_data, rounded_predictions)
        if self._verbose:
            for rec in recalls:
                print(rec)
            print(mrrs[1])
            for ndc in ndcgs:
                print(ndc)
            report_str = 'Test sum {:.2f}, Train sum {:.2f}, Final error {:.5f}, ratio {:.5f}'
            ...
            print(report_str.format(test_sum, train_sum, rmse, ratio))
            return (test_sum, train_sum, rmse, ratio, mrrs[0], ndcgs[0], ndcgs[1])

        """

        print("Recall@[20:200], Mrr@10, ndcg@5, ndcg@10: ")
        for x in range(20, 201, 20):
            recall_at_x = self.evaluator.recall_at_x(x, predictions, self.test_data, rounded_predictions)
            print(recall_at_x)
        ...
        if self._verbose:
            ...
            #print(report_str.format(test_sum, train_sum, rmse, train_recall, test_recall, recall_at_x, ratio, mrr_at_five, ndcg_at_five, mrr_at_ten, ndcg_at_ten))
        ...
        """
            report_str = 'Test sum {:.2f}, Train sum {:.2f}, Final error {:.5f},  ratio {:.5f}'
            print(report_str.format(test_sum, train_sum, rmse, train_recall, test_recall, recall_at_x, ratio, ndcg_at_five, mrr_at_ten, ndcg_at_ten))
            #print(report_str.format(test_sum, train_sum, rmse, ratio), file=sys.stderr)
        return (test_sum, train_sum, rmse, train_recall,test_recall, recall_at_x, ratio, 0, ndcg_at_five,
                mrr_at_ten, ndcg_at_ten)
    ...
    def dump_recommendations(self,file_name, num_recommendations=10 ):
        """
        ...
        base_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))),'recommendations')
